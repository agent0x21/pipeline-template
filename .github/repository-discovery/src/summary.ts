...
export function publishSummary(manifestPath: string): void {
  ...
  if (!summaryPath)
    throw new Error(
      "GITHUB_STEP_SUMMARY is not available; this command must run in GitHub Actions.",
    );
...
export function renderAffectedSummary(data: AffectedManifest): string {
  const rows = data.affectedApplications
    .map(
      (application) =>
        `| ${markdownCell(application.id)} | ${application.reason === "direct-file-change" ? "Direct file change" : "Dependency change"} | ${markdownCell(application.changedFiles.join(", ") || "—")} |`,
    )
    .join("\n");

...
const commitReleaseCandidatesWorkflow = readWorkflow(
  "../../workflows/create-release-candidates-from-main.yml",
);
...
const environmentManifestWorkflow = readWorkflow(
  "../../workflows/generate-environment-manifest.yml",
);
...
describe("environment manifest workflow contract", () => {
  it("runs after successful release-candidate and production-promotion workflows", () => {
    ...
    expect(environmentManifestWorkflow).toContain(
      "Release — Create candidates from a commit",
    );
    expect(environmentManifestWorkflow).toContain(
      "Release — Create a candidate manually",
    );
    expect(environmentManifestWorkflow).toContain(
      "Release — Promote a candidate to production",
    );
    expect(environmentManifestWorkflow).toContain(
  ...
  it("stores the generated JSON as a persistent release asset", () => {
    expect(environmentManifestWorkflow).toContain(
      "environment-manifest-publish-cli.ts",
    );
...
describe("commit-driven release candidate workflow contract", () => {
  it("is manually dispatched with a required commit and builds from that commit", () => {
    ...
    expect(commitReleaseCandidatesWorkflow).toContain(
      "ref: ${{ inputs.commit }}",
    );
  });
});
